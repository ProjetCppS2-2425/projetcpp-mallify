...
void showWindowsToastNotification(const QString &title, const QString &message) {
    ...
    QString psScript = QString(R"(
[Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null
$template = [Windows.UI.Notifications.ToastNotificationManager]::GetTemplateContent([Windows.UI.Notifications.ToastTemplateType]::ToastText02)
$textNodes = $template.GetElementsByTagName("text")
$textNodes.Item(0).AppendChild($template.CreateTextNode("%1")) | Out-Null
$textNodes.Item(1).AppendChild($template.CreateTextNode("%2")) | Out-Null
$notifier = [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("QtApp")
$notification = [Windows.UI.Notifications.ToastNotification]::new($template)
$notifier.Show($notification)
)").arg(escapedTitle, escapedMessage);
    ...
    // Run PowerShell silently in the background
...
void MainWindow::on_btnSubmit_clicked()
{
    ...
    Reclamation rec(-1, name, email, phone, type, priority, description, location, status, 1);
    ...
    if (rec.ajouter()) {
        ...
        showWindowsToastNotification("Success", "Complaint added successfully.");
    } else {
        qDebug() << "Error";

        QMessageBox::critical(this, "Error", "Failed to add complaint.");
         showWindowsToastNotification("Error", "Failed to add complaint.");
    }
}
...
void MainWindow::on_startButton_clicked()
{
    ...
    ui->startButton->setEnabled(false);  // can’t start again until stopped
...
void MainWindow::on_stopButton_clicked()
{
    ...
    ui->stopButton->setEnabled(false);   // can’t stop anymore
...
{
    number = ui->lineEditPhone_2->text();
    if(number == "")
    {
        ui->WarninLineEdit->setText("select a phone number from the table above");
    }
    else
    {
        ui->WarninLineEdit->setText("");
        callNumber(number);
    }
}
...
void MainWindow::on_ShowMap_clicked()
{
    ...
    connect(&dialog, &MapDialog::locationSelected,
            this, &MainWindow::handleLocationSelected);
    dialog.exec();
}
void MainWindow::handleLocationSelected(const QString &address, double latitude, double longitude) {
    ui->lineEditLocation->setText(address);
    // You now have latitude & longitude available too
}
