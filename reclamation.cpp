...
bool Reclamation::exportTraitéeEnPDF(QTableWidget *table, const QString &filePath) {
    ...
    // 2) Set up PDF writer & painter
    QPdfWriter pdf(filePath);
    pdf.setPageSize(QPageSize(QPageSize::A4));
    pdf.setPageMargins(QMargins(40, 60, 40, 40));  // left, top, right, bottom

    QPainter painter(&pdf);
    if (!painter.isActive())
        return false;

    // 3) Determine paintable area (inside margins)
    QRect pageRect = painter.viewport();

    // 4) Load and draw logo at top‑right
    QPixmap logoPixmap("logo.png");
    // If you bundle via .qrc, do: QPixmap logoPixmap(":/assets/logo.png");
    if (!logoPixmap.isNull()) {
        // scale to fixed width, keep aspect ratio
        const int logoW = 1500;
        const int logoH = logoPixmap.height() * logoW / logoPixmap.width();
        const int logoX = pageRect.right() - logoW;
        const int logoY = pageRect.top();
        painter.drawPixmap(QRect(logoX, logoY, logoW, logoH), logoPixmap);
    }
    ...
    // 6) Draw title, positioned below logo if present
    painter.setFont(titleFont);
    painter.setPen(accentColor);
    int titleOffsetY = pageRect.top();
    if (!logoPixmap.isNull()) {
        titleOffsetY += logoPixmap.height() * 800 / logoPixmap.width() + 200;
    } else {
        titleOffsetY += 20;
    }
    painter.drawText(
        QRect(pageRect.x(), titleOffsetY, pageRect.width(), 500),
        Qt::AlignHCenter,
        "Processed Reclamations"
        );
    painter.setPen(textColor);

    // 7) Collect rows marked “Completed”
    QVector<bool> validRows(table->rowCount(), false);
    for (int r = 0; r < table->rowCount(); ++r) {
        auto *item = table->item(r, statusColumn);
        if (item && item->text().trimmed().toLower() == "completed")
            validRows[r] = true;
    }

    // 8) Draw each entry
    int y = titleOffsetY + 800;
    const int x = pageRect.x() + 500;
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        ...
        painter.drawRoundedRect(x - 100, y - 100, contentWidth + 200, 180, 800, 800);
        ...
        for (int col = 0; col < table->columnCount(); ++col) {
            ...
            painter.setPen(textColor);
            painter.drawText(x + 1500, fieldY + 200, value);
        ...
        painter.drawLine(x - 100, fieldY + 100, x + contentWidth + 100, fieldY + 100);

        y = fieldY + 400;
        processedCount++;
    }

    // 9) If nothing to show
    if (processedCount == 0) {
